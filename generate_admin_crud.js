...
const adminDir = path.join(__dirname, 'app', '(main)', 'about-nith');

const generatePage = (pageName, endpoint, itemName, fields) => `
'use client';
import React, { useState, useEffect } from 'react';
import { Plus, Edit, Trash2, Loader2, Save, X } from 'lucide-react';

export default function ${itemName}Page() {
  const API_URL = \`\${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:4000'}/${endpoint}\`;
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold text-gray-800">Manage ${itemName}</h1>
...
const config = {
  'history': ['history', 'History', ['year', 'title_en', 'title_hi', 'description_en', 'description_hi']],
  'core-values': ['core-values', 'CoreValues', ['icon', 'title_en', 'title_hi', 'description_en', 'description_hi']],
  'vision-mission': ['vision-mission', 'Missions', ['icon', 'title_en', 'title_hi', 'description_en', 'description_hi']],
  'connectivity': ['connectivity', 'ConnectivityModes', ['icon', 'title_en', 'title_hi', 'nearest_point_en', 'nearest_point_hi', 'distance_en', 'distance_hi', 'travel_time_en', 'travel_time_hi', 'services_en', 'services_hi', 'additional_info_en', 'additional_info_hi']],
  'about-city': ['about-city', 'CityInfo', ['icon', 'title_en', 'title_hi', 'description_en', 'description_hi', 'image_url']],
};

for (const [folder, [endpoint, itemName, fields]] of Object.entries(config)) {
  const pagePath = path.join(adminDir, folder, 'page.tsx');
  fs.writeFileSync(pagePath, generatePage(folder, endpoint, itemName, fields), 'utf8');
}

// Special case for Goals since it has Goals and Roadmap on the same page
const goalsPage = `
'use client';
import React, { useState, useEffect } from 'react';
...
`;
fs.writeFileSync(path.join(adminDir, 'goals', 'page.tsx'), goalsPage, 'utf8');
