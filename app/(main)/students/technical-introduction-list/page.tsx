...
export default function TechnicalIntroductionListPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      console.error('Error fetching technical intro data:', err);
      setError('Failed to fetch technical data from server. Please verify backend status.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSavePageSettings = async () => {
    ...
    setError(null);
    try {
      const res = await fetch(`${API_URL}/api/student-technical-intro`, {
        method: 'PUT',
      ...
      alert('Technical intro settings saved successfully!');
    } catch (err: any) {
      console.error(err);
  ...
  const handleAddSociety = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`${API_URL}/api/student-technical-intro/societies`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      alert('Technical Society/Club added successfully!');
    } catch (err: any) {
  ...
  const handleSaveSocietyEdit = async (id: number) => {
    ...
    try {
      const res = await fetch(`${API_URL}/api/student-technical-intro/societies/${id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleDeleteSociety = async (id: number) => {
    if (!confirm('Are you sure you want to delete this technical society/club?')) return;
    try {
      await fetch(`${API_URL}/api/student-technical-intro/societies/${id}`, { method: 'DELETE' });
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[400px] gap-3">
        ...
        <p className="text-gray-500 font-medium">Loading Technical Introduction Dashboard...</p>
      </div>
    );
  }
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl sm:text-3xl font-extrabold text-gray-900 tracking-tight">
                Technical Introduction Editor
              </h1>
              <p className="text-sm text-gray-500 mt-1">
                Configure technical banners, about text details, and coordinate technical clubs or societies bilingually.
              </p>
            </div>
          </div>
          <button
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        ...
        <div className="p-6">
          ...
          {activeTab === 'hero' && (
            <div className="space-y-6">
              ...
              <div className="mt-8 p-6 bg-gradient-to-br from-[#631012] to-[#800000] rounded-xl text-white">
                ...
                <h4 className="text-xl sm:text-2xl font-black">{headingData.title_en || 'TECHNICAL — INTRODUCTION'}</h4>
                <p className="text-sm text-white/80 mt-2 leading-relaxed">{headingData.sub_title_en || 'Technical clubs and societies...'}</p>
                <div className="h-px bg-white/10 my-4" />
                <h4 className="text-xl sm:text-2xl font-black">{headingData.title_hn || 'तकनीकी — परिचय'}</h4>
                <p className="text-sm text-white/80 mt-2 leading-relaxed">{headingData.sub_title_hn || 'तकनीकी क्लब...'}</p>
              </div>
            </div>
          )}
          ...
          {activeTab === 'about' && (
            <div className="space-y-6">
              <div className="border-l-4 border-[#631012] pl-3 mb-4">
                <h3 className="font-bold text-gray-800 text-lg">About Technical Activities</h3>
                <p className="text-sm text-gray-500">Edit the detailed information blocks about campus innovative clubs</p>
              </div>
          ...
          {activeTab === 'list' && (
            <div className="space-y-8">
              ...
              {/* Add club form */}
              <form onSubmit={handleAddSociety} className="bg-gray-50/50 border border-gray-200 p-6 rounded-xl space-y-4">
                <h4 className="font-bold text-gray-800 text-sm">Add New Technical Society/Club</h4>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-3 py-2 border border-gray-200 rounded-lg text-xs bg-white text-gray-900"
                      placeholder="e.g. रोबोटिक्स एंड ऑटोमेशन क्लब"
                    />
                  </div>
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-3 py-2 border border-gray-200 rounded-lg text-xs bg-white text-gray-900"
                      placeholder="e.g. Robotics design and competitions"
                    />
                  </div>
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-3 py-2 border border-gray-200 rounded-lg text-xs bg-white text-gray-900"
                      placeholder="e.g. रोबोटिक्स डिजाइन और प्रतियोगिताएं"
                    />
                  </div>
                  <div>
                  ...
                  <div className="md:col-span-2">
                    ...
                    <input
                      ...
                      className="w-full px-3 py-2 border border-gray-200 rounded-lg text-xs bg-white text-gray-900"
                      placeholder="robotics@nit.ac.in"
                    />
                  </div>
                </div>
                ...
                <table className="w-full text-left table-fixed border-collapse text-sm">
                  ...
                  <tbody className="divide-y divide-gray-200 bg-white">
                    {societies.map((soc, idx) => (
                      <tr key={soc.id} className="hover:bg-gray-50/50">
