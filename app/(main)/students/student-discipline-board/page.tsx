...
import { Save, Shield, Plus, Trash2, FileText, Users, Link2 } from 'lucide-react';
...
interface DisciplineData {
  ...
  content: {
    ...
    contact_dean_email: string;
    contact_discipline_email: string;
    rules_url: string;
  };
}

type TabType = 'overview' | 'composition' | 'contacts';

export default function StudentDisciplineBoardPage() {
  const [activeTab, setActiveTab] = useState<TabType>('overview');
  const [loading, setLoading] = useState(true);
  const [data, setData] = useState<DisciplineData>({
    title_en: 'Student Discipline Board',
    title_hi: 'छात्र अनुशासन बोर्ड',
    description_en: '',
    description_hi: '',
    content: {
      overview_en: '',
  ...
  const fetchData = async () => {
    try {
      const res = await fetch('http://localhost:5000/api/v1/students?page_name=discipline-board');
      const json = await res.json();
      if (json.success && json.data) {
        const item = json.data;
        const content = item.content || {};
        setData({
          title_en: item.title_en || 'Student Discipline Board',
          title_hi: item.title_hi || 'छात्र अनुशासन बोर्ड',
          ...
          content: {
            ...
            contact_dean_email: content.contact_dean_email || 'dean.student@nith.ac.in',
            contact_discipline_email: content.contact_discipline_email || 'discipline@nith.ac.in',
            rules_url: content.rules_url || '/student/discipline/rules'
          }
        });
      }
      setLoading(false);
    } catch (err) {
      console.error('Error fetching discipline board data:', err);
      setLoading(false);
    }
  };
  ...
  const updateComposition = (
    ...
    if (!updated[index]) {
      updated[index] = { designation: '', responsibility: '' };
    }
  ...
  const addComposition = () => {
    ...
    updatedEn.push({ designation: '', responsibility: '' });
    updatedHi.push({ designation: '', responsibility: '' });
    setData({
      ...data,
      content: {
        ...data.content,
  ...
  const removeComposition = (index: number) => {
    const updatedEn = (data.content.composition_en || []).filter((_, i) => i !== index);
    const updatedHi = (data.content.composition_hi || []).filter((_, i) => i !== index);
    setData({
      ...data,
      content: {
        ...data.content,
  ...
  const tabs = [
    ...
    { id: 'composition' as TabType, label: 'Committee Composition', icon: <Users size={18} /> },
    { id: 'contacts' as TabType, label: 'Nodal Contacts & Rules', icon: <Link2 size={18} /> },
  ];

  if (loading) {
    return <div className="p-8 text-black font-semibold text-center">Loading Discipline Board Editor...</div>;
  }
    ...
    <div className="space-y-6 p-4 sm:p-6 lg:p-8 text-black bg-[#F8F9FA] min-h-screen">
      ...
      <div className="bg-gradient-to-r from-[#631012] to-[#800000] rounded-2xl shadow-xl p-6 sm:p-8 text-white">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl sm:text-3xl font-extrabold tracking-tight">Student Discipline Board</h1>
              <p className="text-white/80 text-sm sm:text-base mt-1">
                Manage bilingual discipline committee members, overview texts, and nodal configurations.
              </p>
            </div>
          </div>
          <button
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        ...
        <div className="p-4 sm:p-8">
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      className="w-full p-3 border rounded-xl bg-gray-50 focus:ring-2 focus:ring-[#631012] text-black"
                      placeholder="Ensuring a congenial campus environment..."
                    />
                  </div>
                  <div>
                ...
                <div className="space-y-4 border-t md:border-t-0 md:border-l md:pl-6 pt-4 md:pt-0">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className="w-full p-3 border rounded-xl bg-gray-50 focus:ring-2 focus:ring-[#631012] text-black"
                      placeholder="छात्र अनुशासन बोर्ड"
                    />
                  </div>
                  <div>
                    ...
                    <textarea
                      ...
                      className="w-full p-3 border rounded-xl bg-gray-50 focus:ring-2 focus:ring-[#631012] text-black"
                      placeholder="आचरण की स्पष्ट अपेक्षाओं..."
                    />
                  </div>
                  <div>
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-4">
                {Array.from({ length: compositionCount }).map((_, index) => {
                  const enItem = data.content.composition_en[index] || { designation: '', responsibility: '' };
                  const hiItem = data.content.composition_hi[index] || { designation: '', responsibility: '' };
                    ...
                    <div key={index} className="p-6 border rounded-xl bg-gray-50 relative group border-gray-200">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pr-10">
                        ...
                        <div className="space-y-3">
                          <h4 className="text-xs font-bold text-gray-400 uppercase">Member {index + 1} (English)</h4>
                          <div>
                            <label className="block text-xs text-gray-500 mb-1">DESIGNATION / ROLE</label>
                            <input
                              type="text"
                        ...
                        <div className="space-y-3 border-t md:border-t-0 md:border-l md:pl-6 pt-3 md:pt-0">
                          <h4 className="text-xs font-bold text-[#631012] uppercase">सदस्य {index + 1} (हिंदी)</h4>
                          <div>
                            <label className="block text-xs text-gray-500 mb-1">पद / भूमिका (हिंदी)</label>
                            <input
                              type="text"
          ...
          {/* Tab 3: Contacts Section */}
          {activeTab === 'contacts' && (
            <div className="space-y-6 max-w-2xl">
              <h2 className="text-xl font-bold border-b pb-2">Support Contacts & Policy Rules</h2>
              <div className="space-y-4">
                <div>
                  <label className="block text-sm font-semibold text-gray-700 mb-1">Dean Student Welfare Support Email</label>
                  <input
                    type="email"
                    value={data.content.contact_dean_email}
                    onChange={(e) => setData({
                      ...data,
                      content: { ...data.content, contact_dean_email: e.target.value }
                    })}
                    className="w-full p-3 border rounded-xl bg-gray-50 focus:ring-2 focus:ring-[#631012] text-black text-sm"
                    placeholder="dean.student@nith.ac.in"
                  />
                </div>
                <div>
                  <label className="block text-sm font-semibold text-gray-700 mb-1">Faculty Incharge (Discipline) Email</label>
                  <input
                    type="email"
                    ...
                    className="w-full p-3 border rounded-xl bg-gray-50 focus:ring-2 focus:ring-[#631012] text-black text-sm"
                    placeholder="discipline@nith.ac.in"
                  />
                </div>
                <div>
                  <label className="block text-sm font-semibold text-gray-700 mb-1">Disciplinary Rules URL Path</label>
                  <input
                    type="text"
                    value={data.content.rules_url}
                    onChange={(e) => setData({
                      ...data,
                      content: { ...data.content, rules_url: e.target.value }
                    })}
                    className="w-full p-3 border rounded-xl bg-gray-50 focus:ring-2 focus:ring-[#631012] text-black text-sm"
                    placeholder="/student/discipline/rules"
                  />
                </div>
              </div>
            </div>
