...
export default function StudentCouncilBoardPage() {
  const [activeTab, setActiveTab] = useState<TabType>('overview');
  const [loading, setLoading] = useState(true);
  const [data, setData] = useState<CounsellingData>({
    title_en: 'Student Council Board',
    title_hi: 'छात्र परिषद बोर्ड',
    description_en: '',
    description_hi: '',
    content: {
      overview_en: '',
  ...
  const fetchData = async () => {
    try {
      const res = await fetch('http://localhost:5000/api/v1/students?page_name=student-council-board');
      const json = await res.json();
      if (json.success && json.data) {
        const item = json.data;
        const content = item.content || {};
        setData({
          title_en: item.title_en || 'Student Council Board',
          title_hi: item.title_hi || 'छात्र परिषद बोर्ड',
      ...
      console.error('Error fetching council board data:', err);
      setLoading(false);
    }
  };
  ...
  const handleSave = async () => {
    try {
      const payload = {
        page_name: 'student-council-board',
  ...
  if (loading) {
    return <div className="p-8 text-black font-semibold text-center">Loading Council Board Editor...</div>;
  }
      ...
      <div className="bg-gradient-to-r from-[#631012] to-[#800000] rounded-2xl shadow-xl p-6 sm:p-8 text-white">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl sm:text-3xl font-extrabold tracking-tight">Student Council Board</h1>
              <p className="text-white/80 text-sm sm:text-base mt-1">
                Manage student council committee composition, departmental representatives, and support information.
              </p>
            </div>
          </div>
          <button
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      placeholder="Student Council Board"
                    />
                  </div>
                  <div>
                    ...
                    <textarea
                      ...
                      className="w-full p-3 border rounded-xl bg-gray-50 focus:ring-2 focus:ring-[#631012] text-black"
                      placeholder="Student Council Board manages student activities..."
                    />
                  </div>
                  <div>
                ...
                <div className="space-y-4 border-t md:border-t-0 md:border-l md:pl-6 pt-4 md:pt-0">
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      className="w-full p-3 border rounded-xl bg-gray-50 focus:ring-2 focus:ring-[#631012] text-black"
                      placeholder="छात्र परिषद बोर्ड छात्र गतिविधियों..."
                    />
                  </div>
                  <div>
