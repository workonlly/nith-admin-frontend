...
import { UserCog, Plus, Edit2, Trash2, RefreshCw, X, Loader2, Award } from 'lucide-react';
...
export default function FacultyInchargesAdmin() {
  const [records, setRecords] = useState<FacultyInchargeRecord[]>([]);
  ...
  const fetchData = async () => {
    try {
      ...
      console.error('Error fetching faculty incharges:', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const openEditModal = (item: FacultyInchargeRecord) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      let res;
      if (editingId) {
        res = await fetch(`${API_BASE}/api/administration/faculty-incharges/${editingId}`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(payload),
        });
      } else {
        res = await fetch(`${API_BASE}/api/administration/faculty-incharges`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(payload),
        });
  ...
  const handleDelete = async (id: number) => {
    if (!confirm('Are you sure you want to delete this faculty in-charge?')) return;
    try {
      const res = await fetch(`${API_BASE}/api/administration/faculty-incharges/${id}`, { method: 'DELETE' });
      if (res.ok) {
        fetchData();
      } else {
    ...
    <div className="p-6 space-y-6 font-sans">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold text-gray-900">
                Faculty In-Charge Manager
              </h1>
              <p className="text-xs text-gray-500">
                Manage all institute faculty in-charges (NBA, Computer Centre, Purchase, Security, i-STEM, etc.)
              </p>
            </div>
          </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        <div className="bg-[#e9f2f8] border-b border-gray-300 px-6 py-3 text-center">
          <h2 className="text-base font-bold text-[#0c344e]">Faculty In-Charge</h2>
        </div>
              ...
              {loading ? (
                <tr>
                  <td colSpan={6} className="text-center py-12 text-gray-500">
                    ...
                    Loading faculty in-charges...
                  </td>
                </tr>
              ) : records.length === 0 ? (
                <tr>
                  <td colSpan={6} className="text-center py-12 text-gray-500">
                    No records found. Click "Add Faculty In-Charge" to create one.
                  </td>
                </tr>
              ) : (
          ...
          <div className="bg-white rounded-2xl shadow-xl max-w-lg w-full overflow-hidden border border-gray-200">
            <div className="bg-[#500c0e] text-white px-6 py-4 flex items-center justify-between">
              <h2 className="text-base font-bold">
                {editingId ? 'Edit Faculty In-Charge' : 'Add Faculty In-Charge'}
              </h2>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4 max-h-[85vh] overflow-y-auto">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="e.g. Faculty In-Charge NBA"
                />
              </div>
