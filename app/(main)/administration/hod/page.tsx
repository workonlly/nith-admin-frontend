...
import { Building2, Plus, Edit2, Trash2, RefreshCw, X, Loader2, Award } from 'lucide-react';
...
export default function HODAdminPage() {
  const [records, setRecords] = useState<HODRecord[]>([]);
  ...
  const openAddModal = () => {
    ...
    setFormName('');
    setFormDesignation('Associate Professor');
    setFormDepartment('');
    setFormPhone('');
    setFormEmail('');
    setFormFacultyId('');
  ...
  const openEditModal = (item: HODRecord) => {
    setEditingId(item.id);
    setFormSlNo(item.sl_no || '1');
    setFormName(item.name || '');
    setFormDesignation(item.designation || 'Associate Professor');
    setFormDepartment(item.departments || '');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const payload = {
        ...
        sl_no: formSlNo,
        name: formName.trim(),
        departments: formDepartment.trim(),
      ...
      let res;
      if (editingId) {
        res = await fetch(`${API_BASE}/api/administration/hod/${editingId}`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(payload),
        });
      } else {
        res = await fetch(`${API_BASE}/api/administration/hod`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(payload),
        });
      ...
      if (res.ok) {
        alert(editingId ? 'HOD updated!' : 'HOD added!');
        setModalOpen(false);
        fetchData();
      } else {
  ...
  const handleDelete = async (id: number) => {
    if (!confirm('Are you sure you want to delete this HOD record?')) return;
    try {
      const res = await fetch(`${API_BASE}/api/administration/hod/${id}`, { method: 'DELETE' });
      if (res.ok) {
        fetchData();
      } else {
    ...
    <div className="p-6 space-y-6 font-sans">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold text-gray-900">
                Head of Departments (HOD) Manager
              </h1>
              <p className="text-xs text-gray-500">
                Manage Heads of all Academic Departments and Centers at NIT Hamirpur.
              </p>
            </div>
          </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        <div className="bg-[#e9f2f8] border-b border-gray-300 px-6 py-3 text-center">
          <h2 className="text-base font-bold text-[#0c344e]">Head of Departments</h2>
        </div>
          ...
          <table className="w-full text-left text-xs sm:text-sm text-gray-800 border-collapse">
            ...
            <tbody className="divide-y divide-gray-200 bg-white">
              {loading ? (
                ...
                <tr>
                  <td colSpan={7} className="text-center py-12 text-gray-500">
                    No Head of Departments records found. Click "Add Head of Department" to create one.
                  </td>
                </tr>
              ) : (
                records.map((item, idx) => (
                  <tr key={item.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="py-3 px-6 border-r border-gray-200 text-gray-700">
                      {item.designation || 'Associate Professor'}
                    </td>
                    <td className="py-3 px-6 border-r border-gray-200 font-semibold text-[#0c344e]">
                      {item.departments}
                    </td>
          ...
          <div className="bg-white rounded-2xl shadow-xl max-w-lg w-full overflow-hidden border border-gray-200">
            <div className="bg-[#500c0e] text-white px-6 py-4 flex items-center justify-between">
              <h2 className="text-base font-bold">
                {editingId ? 'Edit Head of Department' : 'Add Head of Department'}
              </h2>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4 max-h-[85vh] overflow-y-auto">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="e.g. Dr. Vandna Sharma"
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="e.g. Architecture / Computer Science & Engineering"
                />
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2 text-xs border border-gray-300 rounded"
                    placeholder="head.arch@nith.ac.in"
                  />
                </div>
              </div>
              ...
              <div className="pt-4 border-t border-gray-200 flex items-center justify-end gap-2">
                ...
                >
                  ...
                  <span>{submitting ? 'Saving...' : editingId ? 'Update HOD' : 'Save HOD'}</span>
                </button>
              </div>
            </form>
          </div>
