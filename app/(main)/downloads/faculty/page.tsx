...
const currentMeta = metaData.find(
  (m) => m.page_type === "faculty"
);


  // Helper

  const getCategoryByTab = (tab: TabType): "general" | "cpda" | "fdp" => {
    switch (tab) {
      case "general":
        return "general";
...
useEffect(() => {
  ...
  const fetchDocuments = async () => {
    try {
      ...
      const facultyDocs = data.filter((doc) => doc.type === "faculty");

      const organizedData: DownloadsState = {
        general: facultyDocs.filter((doc) => doc.category_en === "general"),

        cpda: facultyDocs.filter((doc) => doc.category_en === "cpda"),

        conference: facultyDocs.filter((doc) => doc.category_en === "fdp"),
      };
    ...
    <div className="space-y-6 p-4 lg:p-6">
      ...
      <div className="bg-linear-to-r from-[#631012] to-[#7a1214] rounded-lg shadow-lg p-6 text-white">
        <div className="flex items-center gap-3 mb-4">
          ...
          <h1 className="text-3xl font-bold">Faculty Downloads</h1>
        </div>

        <p className="text-white/90">
          Manage Faculty General / CPDA / FDP Forms
        </p>
      ...
      <div className="bg-white rounded-lg shadow-md overflow-hidden">
        ...
        <div className="p-6 border-b border-gray-200 flex items-center justify-between">
          <h2 className="text-xl font-bold text-[#171717]">
            ...
            {activeTab === "conference" && "Conference / FDP"}
          </h2>
