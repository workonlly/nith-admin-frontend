...
useEffect(() => {
  ...
  const fetchDocuments = async () => {
    try {
      ...
      const data: MiscellaneousDocument[] = await res.json();

      // FILTER ONLY MISCELLANEOUS GENERAL
      const filtered = data.filter(
        (doc) => doc.type === "miscellaneous" && doc.category_en === "general",
      );
...
if (Array.isArray(data)) {
  ...
  // Edit
  const handleEdit = (doc: MiscellaneousDocument) => {
    setIsEditing(doc.id);
    if (pdfInputRef.current) pdfInputRef.current.value = "";
    if (wordInputRef.current) wordInputRef.current.value = "";
    setShowForm(true);
...
if (!isEditing) {
  ...
  if (formData.form_type === "both") {
    ...
    try {
      ...
      data.append("category_en", "general");
      ...
      const res = isEditing
        ? await fetch(`${API_BASE}/data/${isEditing}`, {
            method: "PUT",
            body: data,
          })
...
const handleMetaSubmit = async (e: React.FormEvent) => {
  ...
  const currentDocs = documents.general;

  return (
    <div className="space-y-4 sm:space-y-6 p-2 sm:p-4 lg:p-6">
      {/* --- Top Banner --- */}
      <div className="bg-linear-to-r from-[#631012] to-[#7a1214] rounded-lg shadow-lg p-4 sm:p-6 lg:p-8 text-white">
