...
export default function FCPage() {
  ...
  // API Call: Fetch all Finance Committee files
  const fetchFCContent = async () => {
    setLoading(true);
    try {
      ...
      console.error(err);
      addToast('Failed to retrieve Finance Committee records.', 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  const submitNewMember = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) throw new Error('Refused by FC database.');
      ...
      addToast('Registered new FC member successfully!', 'success');
    } catch (err: any) {
  ...
  const triggerDeleteMember = async (id: string) => {
    ...
    try {
      ...
      addToast('Finance Committee member removed successfully.', 'success');
    } catch (err: any) {
  ...
  const submitNewMinute = async (e: React.FormEvent) => {
    ...
    try {
      ...
      addToast('FC Meeting Minute & S3 PDF logged successfully!', 'success');
    } catch (err: any) {
  ...
  const saveMinuteEdits = async (minute: MeetingMinute) => {
    ...
    try {
      ...
      if (!response.ok) throw new Error('Refused to save updates to FC Minutes database.');
      const updatedRecord = await response.json();
    ...
    <div className="space-y-4 sm:space-y-6 p-2 sm:p-4 lg:p-6 min-h-screen bg-[#F4F4F5] relative">
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <div>
              ...
              <p className="text-sm sm:text-base text-white/90 mt-1">
                Connected Administrative Panel: Database & MinIO File Asset Pipelines
              </p>
            </div>
          </div>
          <button
            onClick={fetchFCContent}
            disabled={loading}
      ...
      {loading ? (
        <div className="bg-white rounded-lg shadow-md p-12 text-center flex flex-col items-center justify-center space-y-4">
          ...
          <p className="text-lg font-medium text-stone-600">Retrieving Finance Committee database archives...</p>
        </div>
      ) : (
          ...
          <div className="p-4 sm:p-6 lg:p-8">
              ...
              <div className="space-y-8">
                ...
                <div className="bg-stone-50 border border-stone-200 rounded-xl p-4 sm:p-6 shadow-sm">
                  <h3 className="text-lg font-bold text-[#631012] mb-4 flex items-center gap-2">
                    <Plus className="w-5 h-5 text-[#631012]" /> Add New FC Member
                  </h3>
                  <form onSubmit={submitNewMember} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    ...
                    <div>
                      ...
                      <input
                        ...
                        className="w-full px-3 py-2 border border-stone-300 rounded-lg focus:ring-2 focus:ring-[#631012] focus:border-transparent text-black text-sm"
                        placeholder="e.g. cfo@nith.ac.in"
                      />
                    </div>
                    <div>
                      ...
                      <input
                        ...
                        className="w-full px-3 py-2 border border-stone-300 rounded-lg focus:ring-2 focus:ring-[#631012] focus:border-transparent text-black text-sm"
                        placeholder="e.g. +91-1972-254001"
                      />
                    </div>
                ...
                <div className="space-y-4">
                  <div className="flex items-center gap-2">
                    ...
                    <h2 className="text-xl font-bold text-stone-900">Current FC Directory ({members.length})</h2>
                  </div>

                  {members.length === 0 ? (
                    <div className="text-stone-500 italic p-8 text-center border border-dashed rounded-xl bg-stone-50/50">
                      No FC members registered in the database.
                    </div>
                  ) : (
                    <div className="space-y-4">
            ...
            {activeTab === 'minutes' && (
              <div className="space-y-8">
                ...
                {/* Drag-n-drop file attachment loader for FC meeting mins */}
                <div className="bg-stone-50 border border-stone-200 rounded-xl p-4 sm:p-6 shadow-sm">
                  <h3 className="text-lg font-bold text-[#631012] mb-4 flex items-center gap-2">
                  ...
                  <form onSubmit={submitNewMinute} className="space-y-4">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      <div>
                        ...
                        <input
                          ...
                          placeholder="e.g. 40th Finance Committee Meeting"
                        />
                      </div>
                      <div>
                    ...
                    <div className="border-2 border-dashed border-[#631012]/30 rounded-xl p-4 text-center bg-white relative hover:border-[#631012]/50 transition-colors">
                      <input
                        ...
                        className="hidden"
                      />
                      <label htmlFor="fc-file-uploader" className="cursor-pointer flex flex-col items-center justify-center space-y-2">
