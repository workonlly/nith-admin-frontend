...
const translations: { en: LanguageStrings; hi: LanguageStrings } = {
  en: {
    header: 'News',
    headerSubtitle: 'Manage news section content',
    editorTitle: 'News Editor',
    editorSubtitle: 'Edit news content',
    ...
    titlePlaceholderEn: 'News title in English',
    titleLabelHi: 'Title (Hindi)',
    titlePlaceholderHi: 'News title in Hindi',
    dateLabel: 'Date',
    ...
    descriptionPlaceholderEn: 'News description in English',
  ...
  hi: {
    ...
    newsList: 'समाचार सूची',
    noNews: 'कोई समाचार नहीं मिला',
    addFirstNews: 'पहला समाचार जोड़ें',
    newsLabel: 'समाचार',
...
export default function NewsAdminPage() {
  ...
  const updateNews = (
    ...
    const updated = [...newsData.newss];
    updated[index] = {
      ...updated[index],
      [field]: value,
    };
    setNewsData({ ...newsData, newss: updated });
  };
  ...
  const removeNews = async (index: number) => {
    const newsId = newsData.newss[index].id;
    ...
    if (newsId) {
      try {
        ...
        if (result.success) {
          setNewsData({
            ...newsData,
            newss: newsData.newss.filter((_, i) => i !== index),
          });
        ...
        <div className="p-6 space-y-8">
          ...
          <div className="space-y-5">
            <div className="flex items-center justify-between">
              <h2 className="text-2xl font-bold">{t.newsList}</h2>
              <button
                onClick={addNews}
            ...
            {newsData.newss.length === 0 ? (
              <div className="text-center py-12">
                <p className="text-[#171717]/60 mb-4">{t.noNews}</p>
                <button
                  onClick={addNews}
                  ...
                  >
                    ...
                    <div className="flex justify-between items-center pb-4 border-b border-[#171717]/10">
                      <h3 className="font-semibold text-lg">
                        {t.newsLabel} {index + 1} {news.id && `(ID: ${news.id})`}
                      </h3>
                      <button
                      ...
                      <div className="space-y-4 p-4 bg-white rounded-xl border border-blue-200/50">
                        ...
                        <div>
                          ...
                          <input
                            ...
                            value={news.title_en}
                            onChange={(e) =>
                              updateNews(index, 'title_en', e.target.value)
                            }
                        ...
                        <div>
                          ...
                          <input
                            type="text"
                            value={news.category_en}
                            onChange={(e) =>
                              updateNews(index, 'category_en', e.target.value)
                            }
                        ...
                        <div>
                          ...
                          <textarea
                            ...
                            onChange={(e) =>
                              updateNews(index, 'description_en', e.target.value)
                            }
                      ...
                      <div className="space-y-4 p-4 bg-white rounded-xl border border-orange-200/50">
                        ...
                        <div>
                          ...
                          <input
                            ...
                            value={news.title_hi}
                            onChange={(e) =>
                              updateNews(index, 'title_hi', e.target.value)
                            }
                        ...
                        <div>
                          ...
                          <input
                            type="text"
                            value={news.category_hi}
                            onChange={(e) =>
                              updateNews(index, 'category_hi', e.target.value)
                            }
                        ...
                        <div>
                          ...
                          <textarea
                            rows={3}
                            value={news.description_hi}
                            onChange={(e) =>
                              updateNews(index, 'description_hi', e.target.value)
                            }
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="space-y-3">
                  ...
                  <div className="space-y-3 max-h-96 overflow-y-auto">
                    {newsData.newss.map((news, index) => {
                      const { month, day } = getFormattedDate(news.date);
                      return (
                        <div
                          key={index}
                ...
                <div className="space-y-3">
                  ...
                  <div className="space-y-3 max-h-96 overflow-y-auto">
                    {newsData.newss.map((news, index) => {
                      const { month, day } = getFormattedDateHindi(news.date);
                      return (
                        <div
                          key={index}
                          className="bg-white p-4 rounded-lg border border-gray-200 flex gap-4 items-start hover:shadow-md transition-shadow"
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="mb-2">
                              <span className="inline-block px-2.5 py-1 text-xs font-semibold rounded-md border border-gray-300 text-gray-700 uppercase">
                                {news.category_hi || t.previewCategory}
                              </span>
                            </div>
                            <h3 className="text-sm font-semibold text-[#171717] leading-snug">
                              {news.title_hi || t.previewTitle}
                            </h3>
                            <p className="text-xs text-[#171717]/70 mt-1.5 line-clamp-2">
                              {news.description_hi || t.previewDescription}
                            </p>
                          </div>
                        </div>
                      );
